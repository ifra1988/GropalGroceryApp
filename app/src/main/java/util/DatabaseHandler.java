...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public boolean setCart(HashMap<String, String> map, Float Qty) {
        ...
        if (isInCart(map.get(COLUMN_ID))) {
            String qtyInDb = getInCartItemQty(map.get(COLUMN_ID));
            float totalQty = Integer.parseInt(qtyInDb) + Qty;
            db.execSQL("update " + CART_TABLE + " set " + COLUMN_QTY + " = '" + totalQty + "' where " + COLUMN_ID + "=" + map.get(COLUMN_ID));
            return false;
        } else {
    ...
    public boolean rmvFromCart(HashMap<String, String> map, Float Qty) {
        db = getWritableDatabase();
        if (isInCart(map.get(COLUMN_ID))) {
            String qtyInDb = getInCartItemQty(map.get(COLUMN_ID));
            float totalQty = Integer.parseInt(qtyInDb) - Qty;
    ...
    public int getCartCount() {
        ...
        if (cursor.moveToFirst()){
            quantStr = cursor.getString(cursor.getColumnIndex(COLUMN_QTY));
            totalCount += Integer.parseInt(quantStr) ;
